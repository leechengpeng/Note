...
TCP通过三次握手建立可靠连接：
1. 客户端->服务器发起连接请求：`[SYN], seq = x`，客户端的状态变为**SYN-SENT**
2. 服务器->客户端返回确认信息：`[SYN, ACK], seq = y, ack = x + 1`，服务器状态变为**SYN-RCVD**
3. 客户端->服务器返回确认信息：`[ACK], seq = x + 1, ack = y + 1`，客户端状态变为**ESTABLISHED**，服务器收到确认信息后也变为**ESTABLISHED**状态。
