...
**依赖倒置原则**就是**模块设计依赖于抽象**（抽象类/接口），**不依赖于细节**（实现类，实现接口或者继承抽象类而产生类的细节）。简而言之，就是面向接口编程（面向对象设计的精髓之一）。抽象是对实现的一种约束，约束具体实现细节。**DIP**能够有效降低类之间的**耦合性**，提高系统稳定性，降低并行开发（先接口设计，不会导致团队开发阻塞）引起的风险，提高代码的可读性和可维护性。
